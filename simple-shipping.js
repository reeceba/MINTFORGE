/* MINTFORGE SIMPLE SHIPPING
   Three seller options only:
   FREE SHIPPING, FLAT RATE, WORLDWIDE FLAT RATE.
...
(() => {
  ...
  const countryCode=()=>{const s=$('shipCountry');return String(s?.selectedOptions?.[0]?.dataset?.countryCode||'').toUpperCase()};
  ...
  function calcShipping(){
    ...
    (window.cart||[]).forEach(x=>{
      const p=x.product||{}; const seller=p.seller_wallet||p.id||'seller';
      const charge=shippingForProduct(p);
      bySeller.set(seller,Math.max(bySeller.get(seller)||0,charge));
    });
  ...
  function renderSimpleCart(){
    ...
    $('subtotal').textContent=`$${sub.toFixed(2)} AUD`;
    $('shippingTotal').textContent=ship>0?`$${ship.toFixed(2)} AUD`:'$0.00 AUD';
    $('total').textContent=`$${(sub+ship).toFixed(2)} AUD`;
    $('checkoutBtn').disabled=!c.length||!window.wallet;
    return true;
  }

  function addShippingUI(){
    const m=$('listingModal'); if(!m||m.dataset.mfSimpleShipping)return false;
    m.dataset.mfSimpleShipping='1';
  ...
  function newListingSimple(){
    if(typeof window.newListing==='function')window.newListing();
    setTimeout(()=>{
      addShippingUI();
      if($('mfShipStrategy'))$('mfShipStrategy').value='worldwide_flat';
      if($('mfShipStrategy'))$('mfShipStrategy').dispatchEvent(new Event('change'));
      if($('mfShipAU'))$('mfShipAU').value='0';
      if($('mfShipIntl'))$('mfShipIntl').value='0';
      if($('mfShipPrice'))$('mfShipPrice').value='0';
    },0);
  }

  function editListingSimple(id){
    if(typeof window.editListing==='function')window.editListing(id);
    setTimeout(()=>{
      addShippingUI();
      const p=(window.sellerData?.products||[]).find(x=>x.id===id); if(!p)return;
      const strategy=String(p.shipping_strategy||'free');
      if($('mfShipStrategy'))$('mfShipStrategy').value=['free','flat','worldwide_flat'].includes(strategy)?strategy:'free';
      if($('mfShipPrice'))$('mfShipPrice').value=Number(p.shipping_price||0);
      if($('mfShipAU'))$('mfShipAU').value=Number(p.shipping_price||0);
      if($('mfShipIntl'))$('mfShipIntl').value=Number(p.shipping_override_price||0);
      $('mfShipStrategy')?.dispatchEvent(new Event('change'));
    },0);
  }
  ...
  async function saveListingSimple(){
    try{
      addShippingUI();
      const strategy=$('mfShipStrategy')?.value||'worldwide_flat';
      let domestic=0,intl=0;
      if(strategy==='flat')domestic=Number($('mfShipPrice')?.value||0);
      if(strategy==='worldwide_flat'){domestic=Number($('mfShipAU')?.value||0);intl=Number($('mfShipIntl')?.value||0)}
      ...
      if(strategy==='flat'&&domestic<0||strategy==='worldwide_flat'&&(domestic<0||intl<0))throw Error('Shipping prices cannot be negative.');
      ...
      await api(SELLER_FN,{action:window.editingId?'update_product':'create_product',session_token:sessionToken,product_id:window.editingId,product:p});
      $('listingStatusText').innerHTML='<span class="success">Listing saved ✓</span>';
      await loadSellerData();await loadProducts();setTimeout(closeListing,500);
  ...
  function bind(){
    ...
    addShippingUI();
    window.renderCart=renderSimpleCart;
    window.newListing=newListingSimple;
    window.editListing=editListingSimple;
    window.saveListing=saveListingSimple;
    const nb=$('newListingBtn'),sb=$('saveListing'); if(nb)nb.onclick=newListingSimple;if(sb)sb.onclick=saveListingSimple;
    const sc=$('shipCountry');if(sc&&!sc.dataset.mfSimpleShip){sc.dataset.mfSimpleShip='1';sc.addEventListener('change',()=>setTimeout(renderSimpleCart,0));}
    renderSimpleCart();
    return true;
  }
  wait(bind);
