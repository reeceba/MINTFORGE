...
(() => {
  ...
  function calcShipping(){const bySeller=new Map();cart.forEach(x=>{const p=x.product||{},seller=p.seller_wallet||p.id||'seller',charge=shippingForProduct(p);bySeller.set(seller,Math.max(bySeller.get(seller)||0,charge));});return Number([...bySeller.values()].reduce((a,b)=>a+b,0).toFixed(2));}
  function renderSimpleCart(){
    if(!$('cartItems'))return false;
    const c=cart||[],sub=c.reduce((n,x)=>n+Number(x.product?.price||0)*Number(x.quantity||0),0),ship=calcShipping();
    ...
    $('subtotal').textContent=`$${sub.toFixed(2)} AUD`;$('shippingTotal').textContent=`$${ship.toFixed(2)} AUD`;$('total').textContent=`$${(sub+ship).toFixed(2)} AUD`;$('checkoutBtn').disabled=!c.length||!wallet;return true;
  }
  function addShippingUI(){
    const m=$('listingModal');if(!m)return false;const select=$('mfShipStrategy');if(!select)return false;
  ...
  function newListingSimple(){if(originalNewListing)originalNewListing();setTimeout(()=>{addShippingUI();$('mfShipStrategy').value='worldwide_flat';$('mfShipStrategy').dispatchEvent(new Event('change'));$('mfShipAU').value='0';$('mfShipIntl').value='0';$('mfShipPrice').value='0'},0)}
  function editListingSimple(id){if(originalEditListing)originalEditListing(id);setTimeout(()=>{addShippingUI();const p=(sellerData.products||[]).find(x=>x.id===id);if(!p)return;const strategy=String(p.shipping_strategy||'free');$('mfShipStrategy').value=['free','flat','worldwide_flat'].includes(strategy)?strategy:'free';$('mfShipPrice').value=Number(p.shipping_price||0);$('mfShipAU').value=Number(p.shipping_price||0);$('mfShipIntl').value=Number(p.shipping_override_price||0);$('mfShipStrategy').dispatchEvent(new Event('change'))},0)}
  async function saveListingSimple(){
    try{
      addShippingUI();const strategy=$('mfShipStrategy')?.value||'worldwide_flat';let domestic=0,intl=0;if(strategy==='flat')domestic=Number($('mfShipPrice')?.value||0);if(strategy==='worldwide_flat'){domestic=Number($('mfShipAU')?.value||0);intl=Number($('mfShipIntl')?.value||0)}if(!Number.isFinite(domestic)||!Number.isFinite(intl)||domestic<0||intl<0)throw Error('Shipping prices must be valid numbers.');
      ...
      if(!p.name)throw Error('Product name is required.');if($('mfEnableVariants')?.checked&&typeof mfCollect==='function'){const v=mfCollect();if(!v.variants.length)throw Error('Add at least one variant.');p.variants=v.variants;p.variant_options=v.variant_options}
  ...
  function bind(){
    if(typeof window.api!=='function'||!$('listingModal'))return false;originalNewListing=window.newListing;originalEditListing=window.editListing;addShippingUI();window.renderCart=renderSimpleCart;window.newListing=newListingSimple;window.editListing=editListingSimple;window.saveListing=saveListingSimple;
    const nb=$('newListingBtn'),sb=$('saveListing');if(nb)nb.onclick=newListingSimple;if(sb)sb.onclick=saveListingSimple;const sc=$('shipCountry');if(sc&&!sc.dataset.mfSimpleShip){sc.dataset.mfSimpleShip='1';sc.addEventListener('change',()=>setTimeout(renderSimpleCart,0))}renderSimpleCart();return true;
  }
  wait(bind);
})();
