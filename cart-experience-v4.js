/* MINTFORGE CART — V6 HARD RESET
   One cart controller. Fixes the V5 escaping bug that prevented button IDs/actions
   from being detected. Wallet-gated add-to-cart + direct quantity/remove controls.
*/
(()=>{
 ...
 function clearCart(){
  ...
  if(root){[...root.querySelectorAll('.qty button')].map(itemId).filter(Boolean).forEach(id=>{const f=fn('removeItem');if(f)try{f(id)}catch(e){}});return true}
  return false;
 }
 ...
 function sync(){
  if(!connected()){close();clearCart();const b=$('mfCartCleanButton');if(b)b.style.display='none';}
 ...
 function install(){
  ...
  if(!document.documentElement.dataset.mfCartV6Bound){
   document.documentElement.dataset.mfCartV6Bound='1';
   ...
   const db=$('disconnectBtn');if(db)db.addEventListener('click',()=>setTimeout(()=>{clearCart();sync()},200),true);
  }
  const r=fn('renderCart');if(r&&!r.__mfCartV6){const w=function(){const out=r.apply(this,arguments);setTimeout(()=>{prepare();badge();sync()},0);return out};w.__mfCartV6=true;window.renderCart=w}
  const add=fn('addToCart');if(add&&!add.__mfCartV6){const w=function(){if(!connected()){sync();alert('Connect your wallet before adding items to your cart.');return}const out=add.apply(this,arguments);setTimeout(()=>{prepare();badge();cart.classList.add('mf-cart-clean-open');document.body.style.overflow='hidden'},40);return out};w.__mfCartV6=true;window.addToCart=w}
